    ...
    <>
      ...
      */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t sm:relative sm:border-t-0 sm:border-r w-full sm:w-64 h-16 sm:h-screen flex flex-row sm:flex-col shadow-lg sm:shadow-sm z-40 transition-all">
        ...
        <div className="hidden sm:flex flex-1 flex-col overflow-y-auto py-2">
          {links.map((link) => (
            <NavLink
              key={link.to}
              to={link.to}
              className={({ isActive }) =>
                `flex flex-row items-center px-4 py-2.5 text-xs font-normal transition-colors ${
                  isActive ? 'text-sky-600 bg-sky-50/70 border-l-2 border-sky-600' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50 border-l-2 border-transparent'
                }`
              }
            >
        ...
        <div className="hidden sm:block p-3 border-t border-gray-200">
          <div className="flex items-center mb-3">
            <div className="w-7 h-7 rounded-full bg-sky-100 flex items-center justify-center text-sky-700 font-normal mr-2.5 text-xs">
              {currentUser.name.charAt(0).toUpperCase()}
            </div>
            <div className="overflow-hidden text-xs font-normal">
              <p className="font-normal text-gray-900 truncate">{currentUser.name}</p>
              <p className="text-[11px] text-gray-400 truncate">{currentUser.isAdmin ? 'Administrator' : 'User'}</p>
            </div>
          </div>
          <button
            onClick={logout}
            className="w-full flex items-center justify-center px-3 py-1.5 text-xs font-normal text-gray-700 bg-gray-50 hover:bg-gray-100 rounded border border-gray-200 transition-colors cursor-pointer"
          >
